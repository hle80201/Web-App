...
def read():
    ...
    """
    docs = db.he587.find({}).sort("created_at", -1) # sort in descending order of created_at timestamp
...
def save_message():
    ...
    email = request.form['femail'] # retrieve the data the user entered into the femail field
    brewery = request.form['fbrewery'] # retrieve the data the user entered into the fbrewery field
...
def create_post():
    ...
    db.he587.insert_one(doc_to_insert) # insert a new document
...
def edit(mongoid):
    ...
    """
    doc = db.he587.find_one({"_id": ObjectId(mongoid)})
...
def delete(mongoid):
    ...
    """
    db.he587.delete_one({"_id": ObjectId(mongoid)})
